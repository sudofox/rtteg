...
const connector = connect(
  (state) => {
    return {
      ...
      popupDialogOpen: state.post.popupRepost.popupDialogOpen,
      postId: state.post.popupRepost.postId,
      item: state.post.popupRepost.item,
      sharedObj: state.post.popupRepost.sharedObj,
      user: state.post.popupRepost.user,
      originalUser: state.post.popupRepost.originalUser,
      embededPost: state.post.popupRepost.embededPost,
      embededUser: state.post.popupRepost.embededUser,
      loggedinUserId: state.post.popupRepost.loggedinUserId,
      alreadyReposted: state.post.popupRepost.alreadyReposted,
      hideDropdown: state.post.popupRepost.hideDropdown,
      propOnSubmit: state.post.popupRepost.propOnSubmit,
      toggleOpen: state.post.popupRepost.toggleOpen,
    };
  },
  {repostSubmit, addPost},
);

export const RepostComposerNew = connector(_RepostComposerNew);
function _RepostComposerNew({
  userInfo,
  isPopup,
  trigger,
  propOnSubmit,
  ...
  const handleSubmit = async (filesUploaded) => {
    ...
    if (handleSameText(text, `prevRepost${item.id}`)) {
      setIsLoading(false);
      return;
    }
      ...
      <div className="comment-reposted-post" id="commentRepostedPost">
        {item.classname === "Comment" || item.data?.action === "pub_cm" ? (
          ...
          <FeedItem
            ...
            hasFiles={allFiles.length || gifFile} // TODO:
          />
        )}
      </div>

...
const connector = connect(
  (state) => {
    return {
      ...
      popupDialogOpen: state.post.popupComment.popupDialogOpen,
      postId: state.post.popupComment.postId,
      item: state.post.popupComment.item,
      sharedObj: state.post.popupComment.sharedObj,
      user: state.post.popupComment.user,
      originalUser: state.post.popupComment.originalUser,
      embededPost: state.post.popupComment.embededPost,
      embededUser: state.post.popupComment.embededUser,
      loggedinUserId: state.post.popupComment.loggedinUserId,
      handleStats: state.post.popupComment.handleStats,
      hideDropdown: state.post.popupComment.hideDropdown,
      toggleOpen: state.post.popupComment.toggleOpen,
    };
  },
  {commentSubmit},
);

export const CommentComposerNew = connector(_CommentComposerNew);

function _CommentComposerNew({
  isPopup,
  trigger,
  userInfo,
  popupDialogOpen,
  ...
  commentId,
  handleStats,
  sharedObj, // TODO: old postfeeditem need this
  user,
  originalUser,
  embededPost,
  embededUser,
  ...
  // const [popupDialogOpen, toggleOpen] = useState(false);
  ...
  const handleSubmit = async (filesUploaded) => {
    ...
    if (
      handleSameText(text, `prevComment${postId}`, () => setIsLoading(false))
    ) {
      return;
    }
    ...
    <div className={clsx(classes.root, "comment-wrapper")}>
      ...
      <div className="comment-replied-post" id="commentRepliedPost">
        <FeedItem
          ...
          hideMore={true}
          hidePostStatLine={true}
        />
        <div className="reply-to">
          {t("getter_fe.post.tips.replying_to")}{" "}
          <span>
            @
            {user ? user.ousername || user.data.ousername : userInfo?.ousername}
          </span>
        </div>
